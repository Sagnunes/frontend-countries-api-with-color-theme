...
<template>
  <div class="px-2 mt-6 md:flex md:flex-col md:justify-center md:mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:space-x-16 ">
      <div>
        <img :src="country['flag']" alt="country_flag" class="rounded-2xl mt-8 md:w-[560px] md:h-[401px]">
      </div>
      <div class="mt-6 flex flex-col justify-center md:w-[400px]">
        <h2 class="font-nunito font-extrabold text-lg dark:text-white mb-6">{{ country['name'] }}</h2>
        <div class="flex flex-col md:flex-row md:justify-between">
          <div class="flex flex-col space-y-1 mt-4 md:mt-0">
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Native Name:
              <span class="font-light dark:text-white">{{ country['nativeName'] }}</span>
            </p>
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Population:
              <span class="font-light dark:text-white">{{ country['population'] }}</span>
            </p>
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Region:
              <span class="font-light dark:text-white">{{ country['region'] }}</span>
            </p>
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Sub Region:
              <span class="font-light dark:text-white">{{ country['subregion'] }}</span>
            </p>
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Capital:
              <span class="font-light dark:text-white">{{ country['capital'] }}</span>
            </p>
          </div>
          <div class="mt-8 space-y-2 md:mt-0">
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Top Level Domain:
              <span class="font-light dark:text-white mr-1" v-for="topLevel in country['topLevelDomain']"
                    :key="topLevel">{{ topLevel }}</span>
            </p>
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Currencies:
              <span class="font-light dark:text-white mr-1" v-for="currency in country['currencies']"
                    :key="currency.code">{{ currency.code
                }}</span>
            </p>
            <p class="font-nunito font-semibold text-sm dark:text-white">
              Languages:
              <span class="font-light dark:text-white mr-1" v-for="(language,key) of country['languages']"
                    :key="language.name">
            {{ language.name }}
        ...
        <div class="flex flex-col md:flex-row">
          ...
          <div class="grid grid-cols-3 place-items-center gap-2 mt-4" v-if="country['borders']">
            <div
              ...
              v-for="border in country['borders']" :key="border">{{ border }}
            </div>
          </div>
        </div>
      </div>
